...
export default function ContestsPage() {
    ...
    const handleDelete = async (contest: Contest) => {
        ...
        if (confirmed) {
            try {
                ...
                toastService.success('Contest deleted successfully');
                refresh();
            } catch (error) {
                console.error('Failed to delete contest:', error);
                toastService.error('Failed to delete contest');
            }
        }
    };
    ...
    const handleStatusChange = async (contest: Contest) => {
        ...
        const confirmed = await confirm({
            ...
            message: (
                <span>
                    Are you sure you want to {action} the contest
                    <span className="font-semibold text-foreground"> "{contest.name}"</span>?
                </span>
            ),
        ...
        if (confirmed) {
            try {
                ...
                toastService.success(`Contest ${action}d successfully`);
                refresh();
            } catch (error) {
                console.error(`Failed to ${action} contest:`, error);
                toastService.error(`Failed to ${action} contest`);
            }
        }
    };
